...
exports.registerCompletionProviders = exports.dyeColorPresentationProvider = exports.dyeColorProvider = exports.materialProvider = void 0;
...
const abilityProvider = vscode.languages.registerCompletionItemProvider({ language: "yaml", scheme: "file" }, {
  ...
  cooldown: \${5|5.0,10.0,20.0|}`);
        ...
        if (isInsideAbility) {
            return ["type", "mode", "duration", "damage", "mana", "charge", "cooldown"].map(attr => new vscode.CompletionItem(attr, vscode.CompletionItemKind.Property));
        }
        return [];
    }
});
...
const enchantmentProvider = vscode.languages.registerCompletionItemProvider({ language: "yaml", scheme: "file" }, {
    provideCompletionItems(document, position) {
        ...
        if (!lineAbove.includes("enchants"))
            return [];
        return [
...
// Gợi ý Stats
const statsProvider = vscode.languages.registerCompletionItemProvider({ language: "yaml", scheme: "file" }, {
    provideCompletionItems(document, position) {
        const lineAbove = document.lineAt(position.line - 1).text.trim();
        if (lineAbove.includes("enchants") || lineAbove.includes("ability"))
            return [];
        const stats = [
            // ⚔️ Weapon Stats
...
exports.materialProvider = vscode.languages.registerCompletionItemProvider({ language: "yaml", scheme: "file" }, {
    provideCompletionItems(document, position) {
        const line = document.lineAt(position.line).text.trim();
        // ✅ Chỉ hiển thị gợi ý nếu dòng bắt đầu bằng `material:`
        if (!/^material\s*:\s*$/.test(line))
            return [];
        const materials = [
...
exports.dyeColorProvider = vscode.languages.registerCompletionItemProvider({ language: "yaml", scheme: "file" }, {
    provideCompletionItems(document, position) {
        const lineAbove = document.lineAt(position.line - 1).text.trim();
        if (!lineAbove.startsWith("dye-color:"))
            return [];
        const colors = [
...
exports.dyeColorPresentationProvider = vscode.languages.registerColorProvider({ language: "yaml", scheme: "file" }, {
    provideDocumentColors(document) {
        ...
        while ((match = regex.exec(text))) {
            ...
            const range = new vscode.Range(document.positionAt(match.index + 10), document.positionAt(match.index + match[0].length) // Chỉ tính phần số RGB
            );
...
function registerCompletionProviders(context) {
    context.subscriptions.push(abilityProvider, enchantmentProvider, statsProvider, exports.materialProvider, exports.dyeColorProvider, exports.dyeColorPresentationProvider);
}
